...
async def advanced_lfi_scan(
    ...
    if output_file:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(TOOL_LOG_DIR, f"lfi_scan_{url.replace('/', '_').replace(':', '')}_{timestamp}.json")
            ...
            try:
                ...
                if indicators_found:
                    ...
                    lfi_results.append({
                        ...
                        "file_type": _detect_lfi_file_type(result["stdout"])
                    })
    ...
    try:
        ...
        logger.error(f"Failed to save LFI results: {e}")
    ...
    user_summary = f"Advanced LFI scan complete on {url}. Tested {tested_payloads} payloads."
    if lfi_results:
        user_summary += f"\n🚨 Found {len(lfi_results)} potential LFI vulnerabilities!"
        for result in lfi_results[:3]:
            user_summary += f"\n- Payload: {result['payload']} ({result['confidence']} confidence)"
    else:
        user_summary += "\n✅ No LFI vulnerabilities detected."

    outputs = {"machine_response": response_machine, "user_response": user_summary}
    log_tool_execution("advanced_lfi_scan", inputs, outputs)

    return json.dumps({"machine": response_machine, "user": user_summary}, indent=2)


def _detect_lfi_file_type(content: str) -> str:
    """Detect the type of file based on content for LFI results"""
    content_lower = content.lower()
...
async def nuclei_ai_scan(
    ...
    """
    Advanced Nuclei AI scanning for specific vulnerability patterns.
    Uses AI-powered detection for modern attack vectors.
    """
    inputs = {
    ...
    if output_file:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_query = ai_query.replace(' ', '_').replace('/', '_')[:50]
        output_path = os.path.join(TOOL_LOG_DIR, f"nuclei_ai_{safe_query}_{timestamp}.json")

    cmd = ["nuclei", "-list", target_file, "-ai", ai_query, "-json", "-o", output_path, "-rate-limit", "10"]
    ...
    # Verify output
    verification = verify_output_file(
        output_path, expected_format="text", tool_name="nuclei_ai_scan", min_size_bytes=5
    )

    if not verification["success"]:
        error_msg = f"Nuclei AI scan failed output verification: {verification['error']}"
        response_machine = {
            "status": "failed",
            "error": error_msg,
            "raw_cli_output": result["stdout"],
        ...
        response_user = f"Nuclei AI scan failed. {error_msg}"
        outputs = {"machine_response": response_machine, "user_response": response_user}
        log_tool_execution("nuclei_ai_scan", inputs, outputs)
        return json.dumps({"machine": response_machine, "user": response_user}, indent=2)
    ...
    user_summary = f"Nuclei AI scan complete. Query: '{ai_query}'"
    if findings:
        user_summary += f"\n🎯 Found {len(findings)} potential issues!"
        for finding in findings[:3]:
            info = finding.get('info', {})
            user_summary += f"\n- {info.get('name', 'Unknown')}: {finding.get('matched-at', '')}"
    else:
        user_summary += "\n✅ No issues found matching the AI query."
...
@mcp.tool()
@resolve_references
async def wellknown_files_check(
    url: str,
    output_file: Optional[str] = None,
    ...
    """
    Comprehensive check for well-known sensitive files and directories.
    Includes .git, .env, backup files, config files, and other sensitive paths.
    """
    inputs = {
        "url": url,
        "output_file": output_file,
    ...
    # Well-known sensitive files and paths
    sensitive_paths = [
        ".git/HEAD", ".git/config", ".git/index", ".git/logs/HEAD",
        ".env", ".env.local", ".env.production", ".env.backup",
        "config.json", "config.yaml", "config.php", "application.properties",
        "database.yml", "db_config.php", ".pgpass", ".my.cnf",
        "docker-compose.yml", "kubeconfig", ".dockercfg", ".docker/config.json",
        "id_rsa", "id_rsa.pub", "authorized_keys", "ssh_config",
        "wp-config.php", "wp-config.php.bak", "wp-config.php~",
        ".npmrc", ".yarnrc", "package.json", "composer.json",
        "robots.txt", "sitemap.xml", ".well-known/security.txt",
        "phpinfo.php", "info.php", "test.php", "debug.php",
        "swagger.json", "swagger.yaml", "api-docs", "graphql",
    ...
    if output_file:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(TOOL_LOG_DIR, f"wellknown_scan_{url.replace('/', '_').replace(':', '')}_{timestamp}.json")
                    ...
                    try:
                        ...
                        if status_code == "200" and size > 0:
                            ...
                            sensitive_keywords = [
                                ...
                                'aws_access', 'aws_secret', 'api_key',
                                'private_key', 'root:', 'admin:',
                            ...
                            for keyword in sensitive_keywords:
                                if keyword in response_lower:
                                    is_sensitive = True
                                    content_indicators.append(keyword)
    ...
    try:
        ...
        logger.error(f"Failed to save well-known files results: {e}")

    sensitive_count = len([f for f in findings if f["is_sensitive"]])
    ...
    user_summary = f"Well-known files scan complete on {url}. Tested {len(sensitive_paths)} paths."
    if findings:
        user_summary += f"\n📁 Found {len(findings)} accessible files ({sensitive_count} potentially sensitive)"
        for finding in [f for f in findings if f["is_sensitive"]][:3]:
            user_summary += f"\n🚨 {finding['path']} - Size: {finding['size']} bytes"
        if not sensitive_count:
            for finding in findings[:3]:
                user_summary += f"\n📄 {finding['path']} - Size: {finding['size']} bytes"
    else:
        user_summary += "\n✅ No accessible sensitive files found."

    outputs = {"machine_response": response_machine, "user_response": user_summary}
    log_tool_execution("wellknown_files_check", inputs, outputs)
...
if __name__ == "__main__":
    ...
    for payload in lfi_payloads:
        ...
        try:
            ...
            if any(indicator in result["stdout"].lower() for indicator in [
                "root:x:0:0:", "daemon:", "www-data:", "nobody:",
                "[boot loader]", "# localhost", "127.0.0.1"
            ]):
                lfi_results.append({
                    "payload": payload,
                    "url": test_url,
            ...
            logger.error(f"Error testing LFI payload {payload}: {e}")

    # Save results
    try:
        with open(output_path, 'w') as f:
            json.dump(lfi_results, f, indent=2)
    except Exception as e:
        logger.error(f"Failed to save LFI results: {e}")
    ...
    user_summary = f"Advanced LFI scan complete on {url}. Tested {len(lfi_payloads)} payloads."
    if lfi_results:
        user_summary += f"\n🚨 Found {len(lfi_results)} potential LFI vulnerabilities!"
        for result in lfi_results[:3]:
...
async def nuclei_ai_scan(
    ...
    cmd = ["nuclei", "-list", target_file, "-ai", ai_query, "-json", "-o", output_path]
    ...
    # Verify output
    verification = verify_output_file(
        output_path, expected_format="json", tool_name="nuclei_ai_scan"
    )
    ...
    if findings:
        user_summary += f"\n🎯 Found {len(findings)} potential issues!"
        for finding in findings[:3]:
            user_summary += f"\n- {finding.get('info', {}).get('name', 'Unknown')}: {finding.get('matched-at', '')}"
    else:
        user_summary += "\n✅ No issues found matching the AI query."
...
async def wellknown_files_check(
    ...
    for path in sensitive_paths:
        ...
        try:
            cmd = ["curl", "-s", "-k", "-w", "%{http_code}:%{size_download}", "--max-time", "5", test_url]
            result = run_command(cmd, timeout=10)

            if result["stdout"]:
                parts = result["stdout"].split('\n')
                if len(parts) >= 2:
                    status_info = parts[-1]  # Last line contains status code and size
                    response_body = '\n'.join(parts[:-1])

                    if ':' in status_info:
                        status_code, size = status_info.split(':')

                        if status_code == '200' and int(size) > 0:
                            ...
                            if any(indicator in response_body.lower() for indicator in [
                                ...
                                is_sensitive = True
                                content_indicators = [ind for ind in [
                                    'password', 'secret', 'key', 'token', 'database'
                                ] if ind in response_body.lower()]
...
async def http2_upgrade_test(
    ...
    """
    Tests for HTTP/2 Cleartext (h2c) upgrade vulnerabilities.
    Detects potential WAF bypass opportunities through protocol upgrade.
    """
    inputs = {
        "url": url,
        "output_file": output_file,
    ...
    if output_file:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(TOOL_LOG_DIR, f"h2c_test_{url.replace('/', '_').replace(':', '')}_{timestamp}.json")
    ...
    try:
        ...
        cmd = ["curl", "-s", "-k", "--max-time", "10"]
        for header in h2c_headers:
            cmd.extend(["-H", header])
        cmd.extend(["-w", "%{http_code}:%{size_download}", url])
        ...
        if "101" in result["stdout"]:  # HTTP/1.1 101 Switching Protocols
            results.append({
                "test": "h2c_upgrade",
                "status": "vulnerable",
                "description": "Server accepts H2C upgrade",
                "response": result["stdout"],
                "risk": "High - Potential WAF bypass"
            })
        else:
            results.append({
                "test": "h2c_upgrade",
    ...
    # Test 2: Check for HTTP/2 support indicators
    try:
        ...
        results.append({
            "test": "http2_support",
            "status": "error",
            "description": f"Error testing HTTP/2 support: {e}"
        })

    # Test 3: Protocol confusion test
    if test_bypass:
        try:
            # Send malformed request that might bypass WAF
            malformed_request = "GET / HTTP/1.1\r\nHost: target.com\r\nUpgrade: h2c\r\nConnection: Upgrade\r\n:method: POST\r\n:path: /admin\r\n\r\n"

            cmd = ["curl", "-s", "-k", "--max-time", "10", "--data-raw", malformed_request, url]
            result = run_command(cmd, timeout=15)

            results.append({
                "test": "protocol_confusion",
                "status": "tested",
                "description": "Tested protocol confusion bypass",
                "response_length": len(result["stdout"]),
                "contains_error": "error" in result["stdout"].lower()
            })
            ...
            results.append({
                "test": "protocol_confusion",
                "status": "error",
                "description": f"Error testing protocol confusion: {e}"
            })
    ...
    try:
        ...
        logger.error(f"Failed to save H2C test results: {e}")
    ...
    user_summary = f"HTTP/2 upgrade test complete on {url}."
    if vulnerable_tests:
        user_summary += f"\n🚨 Found {len(vulnerable_tests)} potential H2C vulnerabilities!"
        for vuln in vulnerable_tests:
            user_summary += f"\n- {vuln['description']} ({vuln['risk']})"
    else:
        user_summary += "\n✅ No H2C upgrade vulnerabilities detected."

    outputs = {"machine_response": response_machine, "user_response": user_summary}
    log_tool_execution("http2_upgrade_test", inputs, outputs)
...
async def cve_exploit_check(
    ...
    """
    Checks for recent high-impact CVE vulnerabilities.
    Tests for CVE-2025-57819, CVE-2025-47812, CVE-2025-41646, CVE-2025-32432.
    """
    inputs = {
        "target": target,
        "cve_list": cve_list,
    ...
    if output_file:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(TOOL_LOG_DIR, f"cve_check_{target.replace('/', '_').replace(':', '')}_{timestamp}.json")

    # Recent high-impact CVEs with detection methods
    cve_tests = {
        "CVE-2025-57819": {
            "name": "FreePBX Authentication Bypass",
            "test_path": "/admin/config.php",
            "indicators": ["FreePBX", "Asterisk", "admin"],
            "method": "GET"
        },
        "CVE-2025-47812": {
            "name": "Wing FTP Server RCE",
            "test_path": "/admin",
            "indicators": ["Wing FTP Server", "FTP"],
            "method": "GET"
        },
        "CVE-2025-41646": {
            ...
            "indicators": ["RevPi", "webstatus"],
            "method": "POST"
        },
        "CVE-2025-32432": {
            "name": "CraftCMS RCE",
            "test_path": "/",
            "indicators": ["Craft CMS", "craft"],
            "headers": {"X-Powered-By": "Craft CMS"},
            "method": "GET"
        }
    }

    if cve_list and os.path.exists(cve_list):
        try:
            with open(cve_list, 'r') as f:
                additional_cves = json.load(f)
                cve_tests.update(additional_cves)
        except Exception as e:
            logger.error(f"Failed to load CVE list: {e}")

    results = []
    base_url = target.rstrip('/')

    for cve_id, cve_info in cve_tests.items():
        try:
            test_url = f"{base_url}{cve_info['test_path']}"

            if cve_info["method"] == "GET":
            ...
            if result["stdout"]:
                ...
                status_code = response_body[-3:] if len(response_body) >= 3 else "000"
                content = response_body[:-3] if len(response_body) >= 3 else response_body

                # Check for specific indicators
                indicator_matches = sum(1 for indicator in cve_info["indicators"]
                                      if indicator.lower() in content.lower())

                if indicator_matches > 0:
                    confidence = "medium"
                    if status_code == "200" or "admin" in content.lower():
                        is_vulnerable = True
                        confidence = "high"
            ...
            results.append({
                ...
                "confidence": confidence,
                "status_code": status_code if 'status_code' in locals() else "unknown",
                "indicators_found": indicator_matches if 'indicator_matches' in locals() else 0,
                "response_preview": content[:200] if 'content' in locals() else ""
            })
    ...
    try:
        ...
        logger.error(f"Failed to save CVE check results: {e}")

    vulnerable_cves = [r for r in results if r["vulnerable"]]
    high_confidence = [r for r in vulnerable_cves if r["confidence"] == "high"]

    response_machine = {
        "status": "success",
        "target": target,
        "cves_tested": len(results),
        "potential_vulnerabilities": len(vulnerable_cves),
        "high_confidence_vulns": len(high_confidence),
        "output_file_path": output_path,
        "results": results
    }

    user_summary = f"CVE vulnerability check complete on {target}. Tested {len(results)} CVEs."
    if vulnerable_cves:
        user_summary += f"\n🚨 Found {len(vulnerable_cves)} potential CVE vulnerabilities!"
        for cve in high_confidence:
            user_summary += f"\n🔥 {cve['cve_id']}: {cve['name']} (HIGH confidence)"
        for cve in [c for c in vulnerable_cves if c["confidence"] != "high"][:2]:
            user_summary += f"\n⚠️  {cve['cve_id']}: {cve['name']} ({cve['confidence']} confidence)"
    else:
        user_summary += "\n✅ No known CVE vulnerabilities detected."

    outputs = {"machine_response": response_machine, "user_response": user_summary}
    log_tool_execution("cve_exploit_check", inputs, outputs)
...
@mcp.tool()
@resolve_references
async def advanced_info_gathering(
    target: str,
    gather_emails: bool = True,
    gather_subdomains: bool = True,
    gather_tech_stack: bool = True,
    output_file: Optional[str] = None,
    timeout: int = 900,
) -> str:
    """
    Comprehensive information gathering using multiple techniques.
    Extracts emails, subdomains, tech stack, social media links, and more.
    """
    inputs = {
        "target": target,
        "gather_emails": gather_emails,
        "gather_subdomains": gather_subdomains,
    ...
    if output_file:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(TOOL_LOG_DIR, f"info_gathering_{target.replace('/', '_').replace(':', '')}_{timestamp}.json")
    ...
    try:
        ...
        cmd = ["curl", "-s", "-k", "--max-time", "15", "-L", base_url]
        page_result = run_command(cmd, timeout=20)
        page_content = page_result["stdout"]

        if gather_emails:
            # Extract email addresses
            email_pattern = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'
            emails = list(set(re.findall(email_pattern, page_content, re.IGNORECASE)))
            results["emails"] = emails

        if gather_subdomains:
            # Extract subdomain references
            subdomain_pattern = rf'\b[A-Za-z0-9.-]*\.{re.escape(target.split("//")[-1].split("/")[0])}\b'
            subdomains = list(set(re.findall(subdomain_pattern, page_content, re.IGNORECASE)))
            results["subdomains"] = [s for s in subdomains if s != target.split("//")[-1].split("/")[0]]

        if gather_tech_stack:
            # Detect technologies from headers and content
            header_cmd = ["curl", "-s", "-k", "--max-time", "10", "-I", base_url]
            header_result = run_command(header_cmd, timeout=15)
            headers = header_result["stdout"]

            tech_indicators = {
                "server": re.search(r'Server:\s*([^\r\n]+)', headers, re.IGNORECASE),
                "x_powered_by": re.search(r'X-Powered-By:\s*([^\r\n]+)', headers, re.IGNORECASE),
                "generator": re.search(r'<meta[^>]*name=["\']generator["\'][^>]*content=["\']([^"\']+)["\']', page_content, re.IGNORECASE),
                "framework": []
            }
            ...
            for framework, pattern in framework_patterns.items():
                if re.search(pattern, page_content + headers, re.IGNORECASE):
                    tech_indicators["framework"].append(framework)

            results["tech_stack"] = {k: v.group(1) if hasattr(v
    try:
        mcp.run(transport="stdio")
    except Exception as e:
        logger.critical(f"Server crashed with unhandled exception: {e}")
        raise
